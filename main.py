import torch
from models import SpKBGATModified, SpKBGATConvOnly, GlKBGATModified
...
def parse_args():
    ...
    args.add_argument("-e_go", "--epochs_global_gat", type=int,
    ...
    args.add_argument("-b_gat", "--batch_size_gat", type=int,
                      default=2000, help="Batch size for GAT")
    ...
    args.add_argument("-b_conv", "--batch_size_conv", type=int,
                      default=100, help="Batch size for conv")
...
def train_gat(args):
    ...
    for epoch in range(args.epochs_gat):
        ...
        save_model(model_gat, args.data, epoch,
                   args.output_folder)
        train_global_gat(args, entity_embed, relation_embed, current_batch_2hop_indices)

def train_global_gat(args,entity_embeddn, relation_embeddn,current_batch_2hop_indices):

    print(
        "\nModel type -> GAT layer with {} heads used , Initital Embeddings training".format(args.nheads_GAT[0]))
    model_gat = GlKBGATModified(entity_embeddn, relation_embeddn, args.entity_out_dim, args.entity_out_dim,
                                args.drop_GAT, args.alpha, args.nheads_GAT)
    ...
    current_batch_node2vec_indices, current_batch_node2vec_edge = Corpus_.get_batch_node2vec_all(entity_embeddings,
                                                                                                 relation_embeddings,
                                                                                                 current_batch_2hop_indices)
    current_batch_node2vec_indices = np.array(current_batch_node2vec_indices)
    current_batch_node2vec_edge = np.array(current_batch_node2vec_edge)
    if CUDA:
        current_batch_node2vec_indices = Variable(
            torch.LongTensor(current_batch_node2vec_indices)).cuda()
        current_batch_node2vec_edge = Variable(
            torch.LongTensor(current_batch_node2vec_edge)).cuda()
    #     autograd包是PyTorch中神经网络的核心, 它可以为基于tensor的的所有操作提供自动微分的功能，即可求梯度，反向传播用
    #     torch.LongTensor用于生成一个单精度浮点类型的张量
    else:
        current_batch_node2vec_indices = Variable(
            torch.LongTensor(current_batch_node2vec_indices))
        current_batch_node2vec_edge = Variable(
            torch.LongTensor(current_batch_node2vec_edge))

    epoch_losses = []   # losses of all epochs
    print("Number of epochs {}".format(args.epochs_global_gat))

    for epoch in range(args.epochs_global_gat):
        print("\nepoch-> ", epoch)
        random.shuffle(Corpus_.train_triples)
        Corpus_.train_indices = np.array(
            list(Corpus_.train_triples)).astype(np.int32)
        ...
        for iters in range(num_iters_per_epoch):
            ...
            # forward pass
            entity_embed, relation_embed = model_gat(
                Corpus_, Corpus_.train_adj_matrix, train_indices, current_batch_node2vec_indices, current_batch_node2vec_edge)
