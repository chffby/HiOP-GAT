...
class GlKBGATModified(nn.Module):
    def __init__(self, initial_entity_emb, initial_relation_emb, entity_out_dim, relation_out_dim,
                 drop_GAT, alpha, nheads_GAT):
        '''Sparse version of KBGAT
        entity_in_dim -> Entity Input Embedding dimensions
        ...
        self.num_nodes = initial_entity_emb.shape[0]
        self.entity_in_dim = initial_entity_emb.shape[1]
        self.entity_out_dim_1 = entity_out_dim[1]
        self.nheads_GAT_1 = nheads_GAT[0]
        ...
        self.W_entities = nn.Parameter(torch.zeros(
            size=(self.entity_in_dim, self.entity_out_dim_1 * self.nheads_GAT_1)))
        nn.init.xavier_uniform_(self.W_entities.data, gain=1.414)
        self.W_2hop = nn.Parameter(torch.zeros(
            size=(self.entity_out_dim_1 * self.nheads_GAT_1, self.entity_in_dim)))
    ...
    def forward(self, Corpus_, adj, batch_inputs, train_node2vec_indices,train_node2vec_edge_indices):
        ...
        while i < train_node2vec_indices.shape[1] - 1:
            edge_list_node2vec = torch.cat(
                (train_node2vec_indices[:, i].unsqueeze(-1), train_node2vec_indices[:, i + 1].unsqueeze(-1)),
                dim=1).t()
            edge_type_node2vec = torch.cat(
                (train_node2vec_edge_indices[:, i].unsqueeze(-1),
                 train_node2vec_edge_indices[:, i + 1].unsqueeze(-1)), dim=1)
